    ...
    process_defs = [
        ...
        ("Réponse à une attaque ransomware", "ransomware",
         "Processus de gestion de crise et de remédiation suite à une attaque ransomware", [
             ...
             (2,  "Activer la cellule de crise", "Réunir en urgence : DSI, RSSI, direction générale, responsable juridique. Nommer un coordinateur de crise. Ouvrir un canal de communication hors SI (téléphones mobiles, messagerie externe)."),
             (3,  "Évaluer l'étendue du chiffrement", "Identifier les systèmes et données chiffrés, les partages réseau impactés et les sauvegardes potentiellement compromises. Déterminer le vecteur d'infection initial."),
             (4,  "Notifier les autorités et parties prenantes", "Déposer plainte auprès des autorités (Police/Gendarmerie). Notifier l'ANSSI (si OIV/OSE). Déclarer à la CNIL sous 72h si données personnelles exfiltrées. Informer les assurances cyber."),
             (5,  "Prendre la décision sur la rançon", "Évaluer avec la direction et les assureurs la position sur le paiement de la rançon (déconseillé). Contacter un spécialiste en réponse à incident si nécessaire."),
             (6,  "Éradiquer le ransomware", "Identifier la souche (extension des fichiers, note de rançon). Vérifier l'existence d'un déchiffreur gratuit (No More Ransom). Nettoyer ou réinstaller tous les systèmes compromis depuis des images saines."),
             (7,  "Restaurer depuis les sauvegardes saines", "Identifier la dernière sauvegarde antérieure à l'infection non compromise. Restaurer par ordre de priorité (systèmes critiques en premier). Vérifier l'intégrité de chaque restauration."),
             (8,  "Renforcer et surveiller avant reconnexion", "Appliquer les patches manquants, changer tous les mots de passe, activer le MFA généralisé. Déployer ou renforcer l'EDR. Mettre en place une surveillance renforcée 24/7."),
             (9,  "Reconnexion progressive du SI", "Reconnecter les systèmes par vagues contrôlées en commençant par les plus critiques. Tester chaque service avant d'élargir. Maintenir l'isolation des systèmes non encore nettoyés."),
             (10, "Bilan post-incident et plan de remédiation", "Rédiger le rapport complet (chronologie, impact, coût). Identifier et corriger toutes les failles exploitées. Mettre à jour le PRA/PCA. Organiser un retour d'expérience (REX) avec toutes les équipes."),
         ]),
